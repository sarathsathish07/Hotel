...
const getChatRoomsWithUnreadCount = async (userId) => {
  ...
  const chatRoomsWithUnreadCount = await Promise.all(chatRooms.map(async (room) => {
    const unreadMessagesCount = await messageRepository.countUnreadMessages(room._id);
    return {
      ...room.toObject(),
      unreadMessagesCount,
    };
...
const createOrGetChatRoom = async (userId, hotelId) => {
  let chatRoom = await chatRoomRepository.findChatRoomByUserIdAndHotelId(userId, hotelId);
...
const getHotelChatRooms = async (hotelId) => {
  ...
  const chatRoomsWithUnreadCount = await Promise.all(chatRooms.map(async (room) => {
    const unreadMessagesCount = await chatRoomRepository.countUnreadMessages(room._id);
    return {
      ...room.toObject(),
      unreadMessagesCount,
    };
...
export default { getChatRoomsWithUnreadCount ,createOrGetChatRoom,getHotelChatRooms};
