...
import {
  multerUploadCertificate,
  multerUploadUserProfile,
  multerUploadHotelImages,
  multerUploadRoomImages,
  multerUploadMessageFile,
...
router
  .route("/profile")
  .get(protect, getHotelierProfileHandler)
  .put(
...
router.post(
  "/upload-certificate/:hotelId",
  protect,
  multerUploadCertificate.single("certificate"),
  uploadVerificationDetailsHandler
);
router.post(
  "/add-hotel",
  protect,
  multerUploadHotelImages.array("images", 5),
  addHotelHandler
);
...
router.get("/dashboard", protect, getStatsHandler),
  router
    .route("/hotels/:id")
    .get(protect, getHotelByIdHandler)
    .put(
      protect,
      multerUploadHotelImages.array("images", 5),
      updateHotelHandler
...
router.post(
  "/add-room/:hotelId",
  protect,
  multerUploadRoomImages.array("images", 5),
  addRoom
);
router.put(
  "/rooms/:roomId",
...
router
  ...
  .get(protect, getHotelMessages)
  .post(multerUploadMessageFile.single("file"), protect, sendHotelMessages);
