...
const saveBooking = asyncHandler(async (req, res) => {
  const bookingData = {
    ...
    bookingStatus:
      req.body.paymentStatus === "completed" ? "confirmed" : "pending",
  };
...
const updateBookingStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const hotel = await Hotel.findById(updatedBooking.hotelId).populate(
      "hotelierId"
    );
    if (!hotel) {
      return res.status(404).json({
...
const getBookingsByUserId = asyncHandler(async (req, res) => {
  try {
    const bookings = await bookingService.getBookingsByUserIdService(
      req.user._id
    );
...
const getHotelierBookings = asyncHandler(async (req, res) => {
  try {
    const bookings = await bookingService.getHotelierBookingsService(
      req.hotelier._id
    );
    if (bookings && bookings.length > 0) {
      res.status(200).json({
...
const checkRoomAvailability = asyncHandler(async (req, res) => {
  try {
    const { roomId, checkInDate, checkOutDate, roomCount, guestCount } =
      req.body;
...
const cancelBooking = asyncHandler(async (req, res) => {
  try {
    ...
    const { refundAmount, refundPercentage } =
    ...
    res.status(200).json({
      ...
      message: responseMessages.BOOKING_CANCELLED_SUCCESSFULLY.replace(
        "{refundPercentage}",
        refundPercentage
      ),
    });
...
const cancelBookingByHotelier = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      message: responseMessages.HOTELIER_BOOKING_CANCELLED_SUCCESSFULLY.replace(
        "{refundPercentage}",
        refundPercentage
      ),
    });
